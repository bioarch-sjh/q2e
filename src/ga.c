...
{  
  ...
  for (k = 0; k < numpep; k++)
  {
    ...
    /* print peptide mass for every peptide expected in file to be used for thermal age calculation */
    fprintf(fout2, "%f,", pep[k].pepmass);
    if (pep[k].found == 1)
    {
      printf("%0.1f \n", pep[k].pepmass);
      fprintf(fout, "%0.1f\n", pep[k].pepmass);
        ...
        if (pep[k].sample[i].found == 1)
        {
          ...
          if (bestfit < GALIM) 
          {
            ...
            for (l = 0; l < pep[k].nq; l++)
            {
              ...
              fprintf(fout2, "%0.3f,", bestalpha[l]);
          ...
          else
          {
            ...
            fprintf(fout2, ",");
            if (pep[k].nq > 1) fprintf(fout2, "-1.000 ");
            countminus1++;
          }
        }
        else
    ...
    /*for (l = 0; l < pep[k].nq; l++)
    {
      meanalpha[l] /= (float)(msamples - countminus1);
      stderralpha[l] /= (float)(msamples - countminus1);
      stderralpha[l] = sqrt(stderralpha[l] - meanalpha[l]*meanalpha[l]);
      printf("Mean beta%d is %f with stderr %f ", l+1, meanalpha[l], stderralpha[l]);
    }
    printf(" \n");*/
  }
  fclose(fout);
